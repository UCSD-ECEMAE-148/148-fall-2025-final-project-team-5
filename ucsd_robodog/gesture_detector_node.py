...
def classify_gesture(hand_landmarks):
    ...
    if (finger_states["index"] and finger_states["middle"] and
        not finger_states["ring"] and not finger_states["pinky"]):
        return "TWO"
    if (finger_states["thumb"] and finger_states["pinky"] 
        and not finger_states["middle"] and not finger_states["index"]):
        return "SHAKA"
...
def get_hand_tilt(hand_landmarks):
    fingers = [
        (index_tip, index_ref),
        (middle_tip, middle_ref),
        (ring_tip, ring_ref),
        (pinky_tip, pinky_ref),
    ]

    z_sum = 0 
    for tip, ref in fingers:
        z_sum += (hand_landmarks.landmark[tip].z + hand_landmarks.landmark[ref].z)

    z_avg = z_sum / len(fingers)
    z_tilt_raw = hand_landmarks.landmark[wrist].z - z_avg
    return z_tilt_raw
...
def follow_control(hand_landmarks):
    ...
    zero_tilt = 0.008  # neutral tilt value
    deadzonez = 0.15  # to avoid jitter
    deadzonex = 0.04  # to avoid jitter
...
class GestureDetectorNode(Node):
    ...
    def timer_callback(self):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if gesture_str == "OPEN PALM":
                    ...
                    # Debug overlay (safe to leave even if we don't show the window)
        ...
        # --- X11 / GUI code removed for headless Docker ---
    ...
    def destroy_node(self):
        ...
        if self.cap.isOpened():
            self.cap.release()
        # Safe in headless mode; does nothing if no windows are open
